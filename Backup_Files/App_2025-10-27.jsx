...
import { MapContainer, TileLayer, FeatureGroup, Polyline, Popup, useMap, Rectangle, Polygon, Circle, CircleMarker } from "react-leaflet";
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        ...
        <label
          style={{ padding: "8px 12px", borderRadius: 12, background: "#e5e7eb", cursor: "pointer" }}
        >
          {t.import}
          <input
            type="file"
...
function LocateControl() {
  ...
  const [acc, setAcc] = React.useState(null);      // accuracy radius in meters
  ...
  return (
    <>
      ...
      {/* Button styled like a Leaflet control */}
        ...
        <div className="leaflet-control leaflet-bar">
          {!watching ? (
            <a
              href="#"
              title="Locate me"
              onClick={(e) => { e.preventDefault(); startLocate(); }}
            ...
            <a
              href="#"
              title="Stop locating"
              onClick={(e) => { e.preventDefault(); stopLocate(); }}
              style={{ display: "block", width: 30, height: 30, lineHeight: "30px", textAlign: "center" }}
            >
              ✖️
            </a>
...
function GeomanDraw({ onCreated, onEdited, onDeleted }) {
  ...
  useEffect(() => {
    ...
    // ✅ Show the Geoman toolbar again (top-left)
    map.pm.addControls({
      position: "topleft",
      drawMarker: false,
      ...
      drawCircle: false,
      drawText: false,
      drawPolyline: true,   // Line tool
      drawPolygon: true,
      drawRectangle: true,
      editMode: true,       // edit existing layers
      dragMode: false,
      cutPolygon: false,
      removalMode: true     // delete mode
    });

    // Nice default style for temporary draw layers
    ...
    map.on("pm:create", (e) => {
      ...
      if (shape === "Line") {
        // We render final colored polylines via React, so remove temp layer
      ...
      <div style={{ background:"#fff", borderRadius:16, padding:16, width:"100%", maxWidth:700 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
          ...
          <div style={{ fontSize:12, color:"#6b7280" }}>
            ID: {feature?.properties?._id?.slice(0,8) || "—"}
          </div>
        </div>
          ...
          <label style={{ fontSize:14 }}>Approx. spaces
            <input
              ...
              onChange={(e)=>{
                ...
                setSpaces(v);
                // mark manual override so edits won’t auto-recalc this field
...
// --- Add this helper above JenaParkingMap() ---
function PolylineWithGeoman({ feature, style, onEdit, onDelete, onClick, children }) {
  ...
  useEffect(() => {
    ...
    // --- keep edit OFF by default (no handles shown) ---
    try { if (layer.pm) layer.pm.disable(); } catch {}

    // Wire edit/remove events (they’ll only fire when enabled)
    const handleEdit = () => {
      const geo = layer.toGeoJSON();
    ...
    // Toggle per-layer editing based on the GLOBAL Geoman toolbar state
    const onGlobalEditToggle = (e) => {
      // e.enabled is a boolean provided by Geoman
      try {
        if (e.enabled) {
    ...
    const onGlobalRemovalToggle = (e) => {
      // Nothing to do per-layer; Geoman handles removal UI globally.
      // We keep this here to future-proof if you want to react to it.
    };

    map.on("pm:globaleditmodetoggled", onGlobalEditToggle);
    map.on("pm:globalremovalmodetoggled", onGlobalRemovalToggle);

    return () => {
      layer.off("pm:edit", handleEdit);
      layer.off("pm:remove", handleRemove);
      map.off("pm:globaleditmodetoggled", onGlobalEditToggle);
      map.off("pm:globalremovalmodetoggled", onGlobalRemovalToggle);
  ...
  return (
    <Polyline
      ...
      eventHandlers={{
        ...
        mouseout:  (e) => e.target.closePopup(), // leave -> hide
        click:     (e) => {                      // click -> hide popup + open modal
          e.target.closePopup();
          onClick?.();
        },
      }}
...
export default function JenaParkingMap() {
  ...
  const [lang, setLang] = useState("en");     // default to German if you like
  const t = translations[lang];
  const [panelOpen, setPanelOpen] = useState(true);

  useEffect(() => {
    // Auto-hide the panel on small screens the first time
  ...
  return (
    <div style={{ position:"relative", width:"100%", height:"100vh", overflow:"hidden" }}>
      ...
      <PanelToggle
        ...
        label={panelOpen ? t.ui.hidePanel : t.ui.showPanel}
      />
      <MapContainer
  	...
  	return (
    	 <PolylineWithGeoman
      	  ...
      	  feature={f}
      	  style={style}
      	  onClick={() => { setEditingIndex(idx); setEditingFeature(f); }}  // open modal on click
      	  onEdit={(id, geo) => {
            // recompute length + (auto) spaces
            setFeatures(prev => prev.map(ff => {
              if (ff.properties?._id !== id) return ff;
              const coords = geo.geometry.coordinates;
              const points = coords.map(([lng, lat]) => ({ latitude: lat, longitude: lng }));
              const newLen = getPathLength(points);
              const next = { ...ff, geometry: geo.geometry, properties: { ...ff.properties, length_m: Math.round(newLen) } };
              if (!next.properties.spacesEdited) {
                next.properties.spaces = Math.max(1, Math.round(newLen / 5.5));
              }
              return next;
    ...
    {editingFeature && (
      <MetadataForm feature={editingFeature} onSave={onSaveFeature} onCancel={onCancelEdit} onDelete={deleteFeatureById} />
    )}
